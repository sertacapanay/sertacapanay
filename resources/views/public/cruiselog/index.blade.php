...
<section class="page-hero">
  <div class="wrap">
    ...
    <p class="page-lead" data-tr>Bugüne kadar çıktığım gemi yolculuklarının günlüğü.</p>
    <p class="page-lead b" data-en>A log of the cruises and sea voyages I've taken so far.</p>
  </div>
</section>
...
<section class="sec">
  <div class="wrap">
    ...
    @else
      <div class="clist" id="cruise-list">
        @foreach($cruises as $cruise)
          @php
            $tname   = $isEn ? ($cruise->title_en ?? $cruise->title_tr) : $cruise->title_tr;
            $tfrom   = $isEn ? ($cruise->from_port_en ?? $cruise->from_port_tr ?? '') : ($cruise->from_port_tr ?? '');
            $tto     = $isEn ? ($cruise->to_port_en ?? $cruise->to_port_tr ?? '') : ($cruise->to_port_tr ?? '');
            $tcountry = $isEn ? ($cruise->country_en ?? $cruise->country_tr ?? '') : ($cruise->country_tr ?? '');
          @endphp
          <a href="/{{ $locale }}/cruiselog/{{ $cruise->slug }}" class="ccard">
            <div>
              <div class="c-header">{{ $tname }}</div>
              @if($cruise->cruise_line)
                <div class="c-ship">{{ $cruise->cruise_line }}</div>
              @endif
            </div>
            <div class="c-route">
            ...
            <div class="c-meta">
              @if($cruise->nights)<strong>{{ $cruise->nights }} <span data-tr>gece</span><span data-en>nights</span></strong>@endif
              @if($tcountry){{ $tcountry }}@endif
              @if($cruise->departure_date)
                {{ \Carbon\Carbon::parse($cruise->departure_date)->locale($locale)->isoFormat('D MMM YYYY') }}
              @endif
            </div>
          </a>
        @endforeach
    ...
    <div style="display:flex;justify-content:center;gap:8px;padding:40px 0;font-family:var(--mono);font-size:12px">
      @if($cruises->onFirstPage())<span style="padding:8px 16px;border:1px solid var(--line);border-radius:3px;color:var(--muted)">←</span>
      @else<a href="{{ $cruises->previousPageUrl() }}" style="padding:8px 16px;border:1px solid var(--line);border-radius:3px;color:var(--ink)">←</a>@endif
      @if($cruises->hasMorePages())<a href="{{ $cruises->nextPageUrl() }}" style="padding:8px 16px;border:1px solid var(--line);border-radius:3px;color:var(--ink)">→</a>
